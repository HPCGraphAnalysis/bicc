...
#pragma omp for schedule(guided) nowait
    for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index)
    {
      ...
      for (uint64_t j = 0; j < out_degree; ++j)
      {
        ...
        {
          if(g->local_unmap[vert_index] == 2) {
            std::cout<<"Task "<<procid<<": vertex "<<g->local_unmap[vert_index]<<" being sent because vertex "<<out<<" updated its label, ";
            std::cout<<"out_index = "<<out_index<<" j = "<<j<<", vertex "<<g->local_unmap[vert_index]<<" has degree "<<out_degree<<"\n";
          }
      ...
      if (send) {
        if(g->local_unmap[vert_index] == 2) std::cout<<"Task "<<procid<<": sending vertex "<<g->local_unmap[vert_index]<<"\n";
        add_vid_to_send(&tq, q, vert_index);
        ++tq.thread_queue_size;
      }
    }  
...
#pragma omp for
    for (uint64_t i = 0; i < comm->total_recv; ++i)
    {
      ...
      if(vert_index == NULL_KEY) printf("Rank %d received GID %lu, but it isn't owned\n",procid,comm->recvbuf_vert[i]);
