...
void communicate(dist_graph_t *g, int** labels, std::queue<int>& reg, std::queue<int>& art,uint64_t* potential_artpts){
  ...
  while(updateidx < sendsize){
    ...
    int lid = get_value(g->map,gid);
    Grounding_Status gs = get_grounding_status(labels[lid]);
    //no logic is needed, because all labels in send are accumulated across all ghosts.
    ...
    if(get_grounding_status(labels[lid]) != gs){
      if(potential_artpts[lid])art.push(lid);
      else reg.push(lid);
    }
  }
...
int** bcc_bfs_prop_driver(dist_graph_t *g, uint64_t* potential_artpts, int**labels){
  int done = 0;
